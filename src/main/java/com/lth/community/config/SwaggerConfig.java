...
public class SwaggerConfig {
  ...
  public OpenAPI CommunityOpenAPI() {
    Info info = new Info().version("0.0.1").title("CommunityService").description("""
            ...
            AWS RDS를 이용하여 DB관리 및 EC2를 이용하여 배포, Spring Security를 이용한 비밀번호 암호화,
            JWT를 더하여 Access Token 및 Refresh Token 구현, 이를 통한 인증 및 권한 설정,
            Scheduler를 이용해 일정시간마다 method 실행, 비밀번호 찾기 시 메일로 임시비밀번호 보내기 등 기본적인 CRUD 및 다양한 기능 구현했습니다.
